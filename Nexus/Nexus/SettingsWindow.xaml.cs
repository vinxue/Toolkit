...
namespace Nexus
{
    public partial class SettingsWindow : Window
    {
            ...
            Func<SiteConfig, CoreWebView2BrowsingDataKinds, Task> clearSiteBrowsingDataAsync)
        {
            ...
            _sites.CollectionChanged += Sites_CollectionChanged;
        ...
        private void Sites_CollectionChanged(object? sender, NotifyCollectionChangedEventArgs e) =>
            Dispatcher.BeginInvoke(new Action(UpdateSiteMoveButtonStates));
        ...
        private void MoveSiteDown_Click(object sender, RoutedEventArgs e) =>
            MoveSite(sender, 1);

        private void MoveSite(object sender, int offset)
        {
            if (sender is not FrameworkElement { Tag: SiteConfig site })
            {
                return;
            ...
            int oldIndex = _sites.IndexOf(site);
            int newIndex = oldIndex + offset;
            if (oldIndex < 0 || newIndex < 0 || newIndex >= _sites.Count)
            {
                return;
            }

            _sites.Move(oldIndex, newIndex);
            SiteStore.Save(_sites);
            SitesList.SelectedItem = site;
            UpdateSiteMoveButtonStates();
        }

        private void UpdateSiteMoveButtonStates()
        {
            foreach (var button in FindVisualChildren<Button>(SitesList))
            {
                UpdateSiteMoveButtonState(button);
            }
        }

        private void UpdateSiteMoveButtonState(Button? button)
        {
            if (button?.Tag is not SiteConfig site)
            {
                return;
            }

            int index = _sites.IndexOf(site);
            if (button.Name == "MoveSiteUpButton")
            {
                button.IsEnabled = index > 0;
            }
            else if (button.Name == "MoveSiteDownButton")
            {
                button.IsEnabled = index >= 0 && index < _sites.Count - 1;
            }
        }
        ...
        private void RemoveSite_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (_removeSite(site, dialog.DeleteProfileData))
                {
                    UpdateSiteMoveButtonStates();
                }
            }
        ...
        private CoreWebView2BrowsingDataKinds GetSelectedBrowsingDataKinds()
        {
            ...
            {
                kinds |= CoreWebView2BrowsingDataKinds.AllSite;
            }
            ...
            if (ClearSavedPasswordsBox.IsChecked == true)
            {
        ...
        private IEnumerable<string> GetSelectedBrowsingDataDescriptions()
        {
            ...
            if (ClearCookiesBox.IsChecked == true)
            {
                yield return "- Cookies and site data";
            }
            ...
            if (ClearAutofillBox.IsChecked == true)
            {
                yield return "- Autofill form data";
            }

            if (ClearSavedPasswordsBox.IsChecked == true)
            {
                yield return "- Saved passwords";
            }
        ...
        private static bool IncludesSignOutRisk(CoreWebView2BrowsingDataKinds kinds) =>
            (kinds & (CoreWebView2BrowsingDataKinds.AllSite |
                ...
                CoreWebView2BrowsingDataKinds.Settings)) != 0;
