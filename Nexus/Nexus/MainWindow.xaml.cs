...
namespace Nexus
{
    ...
    {
        private const double SidebarRailWidth = 48;
        private const double SidebarDividerWidth = 1;
        private const double CollapsedContentWidth = SidebarRailWidth;
        private const double ExpandedContentWidth = 220;
        private const double CollapsedWidth = CollapsedContentWidth + SidebarDividerWidth;
        private const double ExpandedWidth = ExpandedContentWidth + SidebarDividerWidth;
        ...
        private void HandleShortcut(KeyEventArgs e, bool defer)
        {
            ...
            Action? action = e.Key switch
            {
                ...
                Key.W when CanCloseActivePage() => () => _ = CloseActivePageAsync(),
                _ => null
            };
        ...
        private bool CanCloseActivePage()
        {
            if (TemporaryAddressBar.Visibility == Visibility.Visible)
            {
                return false;
            }

            if (CanCloseTemporaryPage())
            {
                return true;
            }

            if (SiteList.SelectedItem is not SiteConfig site || !_webViews.TryGetValue(site, out var webView))
            {
                return false;
            }

            return !_isWebContentFullscreen || ReferenceEquals(_fullscreenWebView, webView);
        }
        ...
        private void DeleteSite_Click(object sender, RoutedEventArgs e)
        {
            ...
            PromptRemoveSite(site);
            e.Handled = true;
        }

        private bool PromptRemoveSite(SiteConfig site)
        {
            var dialog = new RemoveSiteWindow(site) { Owner = this };
            return dialog.ShowDialog() == true && RemoveSite(site, dialog.DeleteProfileData);
        }

        private bool RemoveSite(SiteConfig site, bool deleteProfileData)
        {
        ...
        private async Task CloseActivePageAsync()
        {
            if (_isTemporaryPageVisible)
            {
                await CloseTemporaryPageAsync();
                return;
            }
        ...
        private bool DisposeSiteWebView(SiteConfig site)
        {
            if (!_webViews.TryGetValue(site, out var webView))
            {
                return false;
