...
namespace PdfForge
{
    public partial class MainWindow : Window
    {
        ...
        private readonly ObservableCollection<PdfFileItem> _mergeFiles = new();
        ...
        public MainWindow()
        {
            ...
            // Populate watermark font list
            var fonts = PdfService.GetAvailableFonts();
            WmFontFamily.ItemsSource = fonts;
            if (fonts.Count > 0) WmFontFamily.SelectedIndex = 0;
        }
        ...
        private void LangEN_Click(object sender, RoutedEventArgs e) { App.ApplyLanguage("en"); Title = App.S("App_Title"); }
        private void LangZHCN_Click(object sender, RoutedEventArgs e) { App.ApplyLanguage("zh-CN"); Title = App.S("App_Title"); }
        private void LangZHTW_Click(object sender, RoutedEventArgs e) { App.ApplyLanguage("zh-TW"); Title = App.S("App_Title"); }
        ...
        {
            var dlg = new OpenFileDialog { Title = App.S("Dlg_SelectPdfs"), Filter = "PDF Files|*.pdf", Multiselect = true };
            if (dlg.ShowDialog() != true) return;
            foreach (string path in dlg.FileNames)
            {
                if (_mergeFiles.Any(f => f.FilePath == path)) continue;
                var item = new PdfFileItem { FilePath = path };
                try { item.PageCount = _pdf.GetPageCount(path); } catch { item.PageCount = 0; }
                _mergeFiles.Add(item);
            }
            RefreshMergeIndices();
        }

        private void MergeClearAll_Click(object sender, RoutedEventArgs e) => _mergeFiles.Clear();
        ...
        {
            if (_mergeFiles.Count < 2) { MergeStatus.ShowError(App.S("Err_NeedAtLeast2")); return; }
        ...
        {
            MergeEmptyState.Visibility = _mergeFiles.Count == 0 ? Visibility.Visible : Visibility.Hidden;
        }
        ...
        private void RotateBrowseSource_Click(object sender, RoutedEventArgs e)
        {
            ...
            catch { RotatePageInfo.Text = App.S("Rt_UnableToRead"); _rotatePageCount = 0; }
        ...
        private void RotateAllPages_Checked(object sender, RoutedEventArgs e) { if (RotatePages != null) RotatePages.IsEnabled = false; }
        private void RotateAllPages_Unchecked(object sender, RoutedEventArgs e) { if (RotatePages != null) RotatePages.IsEnabled = true; }
        ...
        private async void RotateExecute_Click(object sender, RoutedEventArgs e)
        {
            ...
            int degrees = Rotate90.IsChecked == true ? 90 : Rotate180.IsChecked == true ? 180 : 270;
            List<int>? pages = null;
        ...
        private void OrganizeBrowseSource_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    var d = details[i];
                    string sz = (int)d.WidthPt + " x " + (int)d.HeightPt + " pt" + (d.Rotate != 0 ? $"  (R{d.Rotate})" : "");
                    _organizePages.Add(new PageItem { SourceFile = path, SourcePageIndex = i, DisplayNumber = i + 1, PageLabel = "Page " + (i + 1), SizeInfo = sz });
                }
        ...
        private void OrganizePageList_SelectionChanged(object sender, SelectionChangedEventArgs e) => UpdateOrganizeButtonStates();
        ...
        private void OrganizeDeleteSelected_Click(object sender, RoutedEventArgs e)
        {
            ...
            foreach (var item in selected) _organizePages.Remove(item);
            RefreshOrganizeIndices();
        }
        ...
        {
            InsertSubPanel.Visibility = InsertSubPanel.Visibility == Visibility.Visible ? Visibility.Collapsed : Visibility.Visible;
        }
        ...
        private void InsertBrowseSource_Click(object sender, RoutedEventArgs e)
        {
            ...
            try { int count = _pdf.GetPageCount(path); InsertSourceInfo.Text = count + " pages"; }
            catch { InsertSourceInfo.Text = ""; }
        }

        private void InsertAddToList_Click(object sender, RoutedEventArgs e)
        {
            string srcPath = InsertSourcePath.Text.Trim();
            if (string.IsNullOrEmpty(srcPath) || !File.Exists(srcPath)) { OrganizeStatus.ShowError(App.S("Err_NoInsertSource")); return; }
            ...
            try
            {
                ...
                pages = string.IsNullOrEmpty(rangeStr) ? Enumerable.Range(1, totalPages).ToList() : PdfService.ParsePageRange(rangeStr, totalPages);
            }
            catch (Exception ex) { OrganizeStatus.ShowError(ex.Message); return; }

            int insertAfter = _organizePages.Count;
            string posText = InsertPosition.Text.Trim();
            if (!string.IsNullOrEmpty(posText))
            {
                if (int.TryParse(posText, out int pos)) insertAfter = Math.Max(0, Math.Min(pos, _organizePages.Count));
                else { OrganizeStatus.ShowError("Insert position must be a whole number."); return; }
            }
            ...
            foreach (int pageNum in pages)
            {
                ...
                string sz = (int)d.WidthPt + " x " + (int)d.HeightPt + " pt" + (d.Rotate != 0 ? $"  (R{d.Rotate})" : "");
                _organizePages.Insert(idx++, new PageItem { SourceFile = srcPath, SourcePageIndex = srcIdx, DisplayNumber = 0, PageLabel = "Page " + pageNum, SizeInfo = sz });
            }
            RefreshOrganizeIndices();
        ...
        {
            if (_organizePages.Count == 0) { OrganizeStatus.ShowError(App.S("Err_NoPages")); return; }
            string dst = OrganizeOutputPath.Text.Trim();
            if (!ValidateOutput(dst)) return;

            var sourcePages = _organizePages.Select(p => new KeyValuePair<string, int>(p.SourceFile, p.SourcePageIndex)).ToList();
        ...
        {
            for (int i = 0; i < _organizePages.Count; i++) _organizePages[i].DisplayNumber = i + 1;
        ...
        {
            OrganizeEmptyState.Visibility = _organizePages.Count == 0 ? Visibility.Visible : Visibility.Hidden;
        }
        ...
        private void WmBrowseSource_Click(object sender, RoutedEventArgs e)
        {
            ...
            try { _watermarkPageCount = _pdf.GetPageCount(path); WmPageInfo.Text = string.Format(App.S("Rt_PagesDetected"), _watermarkPageCount); }
            catch { WmPageInfo.Text = App.S("Rt_UnableToRead"); _watermarkPageCount = 0; }
        ...
        {
            if (WmOpacityLabel != null) WmOpacityLabel.Text = (int)e.NewValue + "%";
        }

        private void WmAllPages_Checked(object sender, RoutedEventArgs e) { if (WmPageRange != null) WmPageRange.IsEnabled = false; }
        private void WmAllPages_Unchecked(object sender, RoutedEventArgs e) { if (WmPageRange != null) WmPageRange.IsEnabled = true; }
        ...
        private WatermarkOptions BuildWatermarkOptions()
        {
            ...
            opts.FontFamily = WmFontFamily.SelectedItem as string ?? "SimHei";
            ...
            else { opts.ColorR = 160; opts.ColorG = 160; opts.ColorB = 160; }
            ...
            // Rotation — angles match PDF counter-clockwise convention
            if (WmRotN45.IsChecked == true) opts.Rotation = -45;
            else if (WmRot0.IsChecked == true) opts.Rotation = 0;
            ...
            if (WmAllPagesChk.IsChecked != true)
            {
                ...
                if (string.IsNullOrEmpty(rangeStr)) throw new ArgumentException(App.S("Err_EnterPagesOrAllWm"));
        ...
        private void EncBrowseSource_Click(object sender, RoutedEventArgs e)
        {
            ...
            try { int count = _pdf.GetPageCount(path); EncPageInfo.Text = string.Format(App.S("Rt_PagesDetected"), count); }
        ...
        private async void EncExecute_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (!ValidateSource(src)) return;
            if (string.IsNullOrEmpty(userPwd) && string.IsNullOrEmpty(ownerPwd)) { SecurityStatus.ShowError(App.S("Err_NoPwdProvided")); return; }
            if (!string.IsNullOrEmpty(userPwd) && userPwd != userPwd2) { SecurityStatus.ShowError(App.S("Err_PwdMismatch")); return; }
            if (!ValidateOutput(dst)) return;
        ...
        private async void RemExecute_Click(object sender, RoutedEventArgs e)
        {
            ...
            catch (Exception ex) { SecurityStatus.ShowError(string.Format(App.S("Err_RemovePwdFailed"), ex.Message)); }
        ...
        private void SplitBrowseSource_Click(object sender, RoutedEventArgs e)
        {
            ...
            try { _splitPageCount = _pdf.GetPageCount(path); SplitPageInfo.Text = string.Format(App.S("Rt_PagesDetected"), _splitPageCount); }
            catch { SplitPageInfo.Text = App.S("Rt_UnableToRead"); _splitPageCount = 0; }
        ...
        {
            var dlg = new OpenFolderDialog { Title = App.S("Dlg_SelectFolder") };
            if (dlg.ShowDialog() == true)
                SplitOutputFolder.Text = dlg.FolderName;
        }
        ...
        private async void SplitExecute_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (!ValidateSource(src)) return;
            if (_splitPageCount == 0) { SplitStatus.ShowError(App.S("Err_NoSourcePdf")); return; }
            if (string.IsNullOrWhiteSpace(folder)) { SplitStatus.ShowError(App.S("Err_NoOutputFolder")); return; }
            if (string.IsNullOrWhiteSpace(prefix)) { SplitStatus.ShowError(App.S("Err_NoPrefix")); return; }
            ...
            try
            {
                ...
                {
                    if (!int.TryParse(SplitPagesPerFile.Text, out int ppf) || ppf < 1) { SplitStatus.ShowError(App.S("Err_NoPagesPerFile")); return; }
                    outputFiles = await Task.Run(() => _pdf.SplitPdfByCount(src, ppf, folder, prefix));
                }
                else
                {
                    string ranges = SplitRangesInput.Text.Trim();
                    if (string.IsNullOrEmpty(ranges)) { SplitStatus.ShowError(App.S("Err_NoRanges")); return; }
        ...
        private void ExtractPanel_Drop(object sender, DragEventArgs e)
        {
            ...
            if (path == null) return;
            ExtractSourcePath.Text = path;
            try { _extractPageCount = _pdf.GetPageCount(path); ExtractPageInfo.Text = string.Format(App.S("Rt_PagesDetected"), _extractPageCount); }
            catch { ExtractPageInfo.Text = App.S("Rt_UnableToRead"); _extractPageCount = 0; }
            if (string.IsNullOrWhiteSpace(ExtractOutputPath.Text))
                ExtractOutputPath.Text = Path.Combine(Path.GetDirectoryName(path)!, Path.GetFileNameWithoutExtension(path) + "_extracted.pdf");
        }
        ...
        private void RotatePanel_Drop(object sender, DragEventArgs e)
        {
            ...
            if (path == null) return;
            RotateSourcePath.Text = path;
            try { _rotatePageCount = _pdf.GetPageCount(path); RotatePageInfo.Text = string.Format(App.S("Rt_PagesDetected"), _rotatePageCount); }
            catch { RotatePageInfo.Text = App.S("Rt_UnableToRead"); _rotatePageCount = 0; }
            if (string.IsNullOrWhiteSpace(RotateOutputPath.Text))
                RotateOutputPath.Text = Path.Combine(Path.GetDirectoryName(path)!, Path.GetFileNameWithoutExtension(path) + "_rotated.pdf");
        }
        ...
        private void WatermarkPanel_Drop(object sender, DragEventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(WmOutputPath.Text))
                WmOutputPath.Text = Path.Combine(Path.GetDirectoryName(path)!, Path.GetFileNameWithoutExtension(path) + "_watermarked.pdf");
        }
        ...
        private void MetadataPanel_Drop(object sender, DragEventArgs e)
        {
            ...
            MetaPageInfo.Text = App.S("Rt_ClickLoadMeta");
            if (string.IsNullOrWhiteSpace(MetaOutputPath.Text))
                MetaOutputPath.Text = Path.Combine(Path.GetDirectoryName(path)!, Path.GetFileNameWithoutExtension(path) + "_metadata.pdf");
        }
        ...
        private void SecurityPanel_Drop(object sender, DragEventArgs e)
        {
            ...
            if (isEncrypt)
            {
                ...
                if (string.IsNullOrWhiteSpace(EncOutputPath.Text))
                    EncOutputPath.Text = Path.Combine(Path.GetDirectoryName(path)!, Path.GetFileNameWithoutExtension(path) + "_encrypted.pdf");
            }
            else
            {
                RemSourcePath.Text = path;
                RemPageInfo.Text = App.S("Rt_EnterPwdToRemove");
                if (string.IsNullOrWhiteSpace(RemOutputPath.Text))
                    RemOutputPath.Text = Path.Combine(Path.GetDirectoryName(path)!, Path.GetFileNameWithoutExtension(path) + "_decrypted.pdf");
            }
        }
        ...
        private static void SetBusy(Button btn, bool busy)
        {
            ...
            {
                if (btn.Tag is string original) btn.Content = original;
                btn.Tag = null;
            }
        }
    }
